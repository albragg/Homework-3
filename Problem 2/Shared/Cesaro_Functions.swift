...
///
/// - Returns: the new angle in which to progress the fractal -> now gives a random angle
///
func turn () -> CGFloat
{
    let newangle = Double.random(in: 0...360)
...
func CesaroFractalCalculator(fractalnum: Int, x: CGFloat, y: CGFloat, size: Double, angleDivisor: Int) -> [(xPoint: Double, yPoint: Double)]
{
   ...
   // var angleChange: CGFloat = 0.0
    ...
    // Calculates Side 1
   // angleChange = 120.0
    allThePoints += calculateCesaroSide(&angle, fractalnum, &myX, &myY, size, angleDivisor)
...
///
func calculateCesaroSide(_ angle: inout CGFloat, _ fractalnum: Int, _ myX: inout CGFloat, _ myY: inout CGFloat, _ size: Double, _ angleDivisor: Int) -> [(xPoint: Double, yPoint: Double)] {
...
func CesaroSide(fractalnum: Int, x: CGFloat, y: CGFloat, angle: CGFloat, size: Double, divisorForAngle: Int) -> [(xPoint: Double, yPoint: Double)] {
    ...
    if(fractalnum == 0){
      ...
      //  let newSizeOfSide = size/(2.0*(1.0+sin(((theta))/2.0)))
        for _ in 1...10 {
        let newCurrentPoint = CesaroSide(fractalnum: fractalnum-1, x: myX, y: myY, angle: myAngle, size: size * n, divisorForAngle: piDivisorForAngle)
            currentPoint += newCurrentPoint
            myX = CGFloat(currentPoint[currentPoint.endIndex-1].xPoint)
            myY = CGFloat(currentPoint[currentPoint.endIndex-1].yPoint)
        
            myAngle = turn()
            totalIterationsInWall += 1
            n *= 0.9
        }
       /* newSizeOfSide = size*0.1
